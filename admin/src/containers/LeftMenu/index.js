...
const LeftMenu = forwardRef(({ shouldUpdateStrapi, version, plugins }, ref) => {
  ...
  const generalSectionLinksFiltered = useMemo(
    () => filterLinks(generalSectionLinks),
    [generalSectionLinks]
  );
  const pluginsSectionLinksFiltered = useMemo(
    () => filterLinks(pluginsSectionLinks),
    [pluginsSectionLinks]
  );

  const singleTypesSectionLinksFiltered = useMemo(
    () => filterLinks(singleTypesSectionLinks),
    [singleTypesSectionLinks]
  );
  const collectTypesSectionLinksFiltered = useMemo(
    () => filterLinks(collectionTypesSectionLinks),
    [collectionTypesSectionLinks]
  );
  ...
  const getModels = async () => {
    ...
    try {
      ...
      const singleTypesSectionResults = await Promise.all(
  ...
  useEffect(() => {
    const getLinksPermissions = async () => {
      const generalSectionLinksArrayOfPromises = generateArrayOfPromises(
        generalSectionLinks
      );
      const pluginsSectionLinksArrayOfPromises = generateArrayOfPromises(
        pluginsSectionLinks
      );

      await getModels();

      const generalSectionResults = await Promise.all(
        generalSectionLinksArrayOfPromises
      );
      const pluginsSectionResults = await Promise.all(
